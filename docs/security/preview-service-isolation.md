...
## Firebase public Auth isolation configuration (2026-09-24)

The Preview Firebase Web App is registered only in `cothecoconutcompany-preview` (project number `125954607774`), with App ID `1:125954607774:web:e60e24ccdd8fc95d8bc98f`. Vercel **Preview-only** public client variables now use that app's project ID, Auth domain, API key, App ID, messaging sender ID, and storage bucket. The Firebase Management API returned these fields together for this one Preview Web App. The Production public variables were not changed.

Firebase Authentication was initialized only in the Preview project through Firebase CLI. Email/password is enabled with a password required; no federated, anonymous, or phone provider is enabled. The only authorized domains are the project's default `cothecoconutcompany-preview.firebaseapp.com` and `cothecoconutcompany-preview.web.app`. No user was created or imported, and no password-reset message was sent. Customer login, registration, OTP, and password reset use DEV Cognito, while `components/admin/AdminForms.tsx` uses Firebase email/password for admin login and reset. The unused Google provider helper does not establish a Google sign-in flow.

The source now rejects a Preview Firebase Auth project other than `cothecoconutcompany-preview`, rejects a mismatch between the server-side public and Admin project IDs, and rejects the Preview project in Production. Firebase-dependent mutable actions fail closed on mismatch; public content can use its fallback when Admin is unavailable. Rate-limit document IDs and in-process keys begin with `preview:`, `production:`, or `development:`; new rate-limit documents and security events also record `environment`. Historical documents are untouched. Preview Admin credentials remain in the existing isolated service identity with access only to the Preview project.

A fresh immutable Vercel Preview from the final source SHA is required to prove the deployed client bundle, Admin runtime identity, and Production unchanged before Gate A is marked passed. No authenticated customer action is authorized until that runtime proof is complete. The older audit below is retained as provenance for the prior blocked state.
