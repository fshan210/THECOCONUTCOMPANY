      ...
      <main>
        ...
        <section id="all-products" className="px-4 py-8 md:px-8 md:py-12">
          <div className="mx-auto max-w-[1320px]">
            <div className="mb-6 flex items-center gap-4 rounded-[22px] border border-black/6 bg-white/48 p-3 shadow-[0_12px_40px_rgba(42,27,19,.04)] backdrop-blur-xl">
              <div className="relative min-w-0 flex-1"><Search className="absolute left-4 top-1/2 -translate-y-1/2" size={16} /><input value={search} onChange={(event) => { setSearch(event.target.value); setActiveSuggestion(0); }} onFocus={() => setSearchFocused(true)} onBlur={() => window.setTimeout(() => setSearchFocused(false), 150)} onKeyDown={(event) => { if (event.key === "ArrowDown") setActiveSuggestion((value) => Math.min(value + 1, suggestions.length - 1)); if (event.key === "ArrowUp") setActiveSuggestion((value) => Math.max(value - 1, 0)); if (event.key === "Enter" && suggestions[activeSuggestion]) { setSearch(suggestions[activeSuggestion].name); setSearchFocused(false); } }} placeholder="Search .CO products" aria-label="Search products" className="h-11 w-full rounded-full bg-[#f8f4ec] pl-11 pr-4 text-xs outline-none" />
                <AnimatePresence>{searchFocused && <motion.div initial={{opacity:0,y:-6}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6}} className="absolute left-0 right-0 top-[calc(100%+8px)] z-40 rounded-[20px] border border-white/70 bg-[rgba(248,244,236,.94)] p-2 shadow-[0_22px_65px_rgba(42,27,19,.15)] backdrop-blur-2xl"><p className="px-3 py-2 text-[9px] font-semibold uppercase text-[#75695f]">{search ? "Suggestions" : "Popular products"}</p>{(search ? suggestions : products.slice(0,4)).map((item,index)=><button type="button" key={item.slug} onMouseDown={()=>setSearch(item.name)} className={cn("flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-xs",activeSuggestion===index&&"bg-white/75")}><span className="relative size-10 overflow-hidden rounded-lg"><ProductImage product={item} sizes="40px" /></span>{item.name}</button>)}</motion.div>}</AnimatePresence>
              </div>
