...
const milestones = [
  { year: "2020", title: "It all started", body: "A simple idea: build a coconut company around considered products and everyday living.", icon: Sprout, image: websiteAssets.about.journey[0] },
  { year: "2021", title: "Building the foundation", body: "Early product work turned the idea into a practical sourcing and production plan.", icon: TreePalm, image: websiteAssets.about.journey[1] },
  { year: "2022", title: "First product direction", body: "The coconut-water ecosystem became the anchor for the wider .CO brand world.", icon: Droplets, image: websiteAssets.about.journey[2] },
  { year: "2023", title: "Growing the ecosystem", body: "Kitchen, Creamery and Botanica directions expanded the brand beyond one bottle.", icon: Heart, image: websiteAssets.about.journey[3] },
  { year: "2024", title: "Rooted partnerships", body: "A Pollachi contract-farm anchor and Kerala operating base shaped the sourcing model.", icon: HandHeart, image: websiteAssets.about.journey[4] },
  { year: "Next", title: "Made for living", body: "Phase-one UHT production and a measured VAP network are the next execution milestones.", icon: Package, image: websiteAssets.about.journey[5] },
] as const;
          ...
          <AnimatePresence mode="wait">
            <motion.article key={current.year} className="relative flex min-h-[440px] flex-col justify-between" initial={{ opacity: 0, y: 22, filter: "blur(7px)" }} animate={{ opacity: 1, y: 0, filter: "blur(0px)" }} exit={{ opacity: 0, y: -16, filter: "blur(5px)" }} transition={{ duration: .54, ease: motionEase }}>
              ...
              <div className="relative my-7 aspect-[4/3] overflow-hidden rounded-[28px] border border-white/65 bg-[#f7f2e8] shadow-[0_18px_48px_rgba(53,39,30,.08)]">
                <Image src={current.image.desktop} alt={current.image.alt} fill sizes="(min-width: 768px) 680px, 92vw" placeholder="blur" blurDataURL={blurDataURL} className="object-cover" />
              </div>
