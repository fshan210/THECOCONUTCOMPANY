...
function Scene({ variant }: { variant: AuthVariant }) {
  ...
  return <aside className="co-auth-scene" aria-label="A sunlit .CO coconut ritual" style={{ "--auth-scene-desktop": `url(\"${background.desktop}\")`, "--auth-scene-mobile": `url(\"${background.mobile}\")` } as React.CSSProperties}><blockquote className="co-auth-scene-quote"><p>{authQuotes[variant]}</p><span aria-hidden="true" /><cite>The .CO way</cite></blockquote>{(variant === "login" || variant === "register") && <BenefitPanel register={variant === "register"} />}</aside>;
}
