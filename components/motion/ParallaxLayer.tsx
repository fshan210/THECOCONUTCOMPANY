...
export function ParallaxLayer({ children, distance = 16, className = "" }: { children: ReactNode; distance?: number; className?: string }) { const ref = useRef<HTMLDivElement>(null); const quality = useMotionQuality(); const { scrollYProgress } = useScroll({ target: ref, offset: ["start end", "end start"] }); const y = useTransform(scrollYProgress, [0, 1], quality === "full" ? [-distance, distance] : [0, 0]); return <motion.div ref={ref} style={{ y }} className={className}>{children}</motion.div>; }
