...
export function UtilityPage({ page }: { page: LaunchPage }) {
  ...
  return <div className="min-h-[70vh] bg-[#f8f4ec] px-4 pb-20 pt-8 md:px-8 md:pt-14"><div className="mx-auto max-w-[1180px]"><p className="text-[10px] font-semibold uppercase tracking-[.16em] text-[#214d2b]">{page.eyebrow}</p><h1 className="mt-5 max-w-[15ch] font-['Cormorant_Garamond'] text-[52px] leading-[.9] tracking-[-.035em] text-[#2a1b13] md:text-[76px]">{page.title}</h1><p className="mt-6 max-w-[58ch] text-sm leading-7 text-[#62574e]">{page.intro}</p>
    {page.mode === "search" ? <div className="mt-10"><label htmlFor="site-search" className="sr-only">Search the .CO website</label><div className="flex min-h-14 items-center rounded-full border border-black/8 bg-white/65 px-5 shadow-sm"><Search size={18}/><input id="site-search" autoFocus value={query} onChange={(event)=>setQuery(event.target.value)} placeholder="Products, recipes, stories…" className="min-w-0 flex-1 bg-transparent px-4 text-sm outline-none"/></div>{results.length ? <div className="mt-5 grid gap-3 md:grid-cols-2">{results.map(([title,href,type])=><Link key={href} href={href} className="flex min-h-20 items-center justify-between rounded-[22px] border border-black/6 bg-white/55 p-5"><span><b className="block text-sm">{title}</b><small className="mt-1 block text-[#6b6057]">{type}</small></span><ArrowRight size={17}/></Link>)}</div> : <StatePanel className="mt-5" kind="empty" title="No .CO results yet." body="Try coconut water, recipes, sustainability or founders." onPrimary={{label:"Clear search",action:()=>setQuery("")}} secondary={{label:"Browse products",href:"/shop"}} />}</div> : null}
    ...
    {page.mode === "cart" ? <StatePanel className="mt-10 max-w-[620px]" compact kind="empty" eyebrow="Your basket" title="Your coconut shelf is empty." body="Add a product from the shop and it will wait here while you explore." primary={{label:"Shop products",href:"/shop"}} secondary={{label:"Find a recipe",href:"/recipes"}} /> : null}
