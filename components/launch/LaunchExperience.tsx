...
import { Check, Cookie, Gift, Leaf, Settings2, WifiOff, X } from "lucide-react";
import { usePathname, useRouter } from "next/navigation";
import { useEffect, useState } from "react";
import { OPEN_CONSENT_EVENT, readCookieConsent, saveCookieConsent } from "@/lib/privacy/consent";
...
export function LaunchExperience() {
  ...
  useBodyScrollLock(preferencesOpen || welcomeOpen);
  ...
  useEffect(() => {
    const markFailedImage = (event: Event) => {
      const image = event.target;
      if (!(image instanceof HTMLImageElement) || !image.parentElement) return;
      image.parentElement.dataset.imageError = "true";
      image.style.opacity = "0";
    };
    document.addEventListener("error", markFailedImage, true);
    return () => document.removeEventListener("error", markFailedImage, true);
  }, []);
    ...
    <>
      ...
      <Dialog.Root open={preferencesOpen} onOpenChange={setPreferencesOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-[250] max-h-[calc(100dvh-24px)] w-[min(92vw,520px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto overscroll-contain rounded-[28px] border border-white/70 bg-[#f8f4ec] p-6 shadow-[0_35px_110px_rgba(22,15,10,.32)] [touch-action:pan-y] focus:outline-none">
      ...
      <Dialog.Root open={welcomeOpen} onOpenChange={setWelcomeState}>
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed left-1/2 top-1/2 z-[230] max-h-[calc(100dvh-24px)] w-[min(92vw,780px)] -translate-x-1/2 -translate-y-1/2 overflow-y-auto overscroll-contain rounded-[30px] border border-white/70 bg-[#f8f4ec] shadow-[0_38px_120px_rgba(22,15,10,.34)] [touch-action:pan-y] focus:outline-none md:grid md:grid-cols-[.88fr_1.12fr]">
            <div className="relative hidden min-h-[520px] overflow-hidden bg-cover bg-center md:block" style={{ backgroundImage: `url("${mediaUrl("/assets/home/refined/naturally-hydrating-4k.png")}")` }}><div className="absolute inset-0 bg-gradient-to-t from-[#17361d]/80 via-transparent to-white/10"/><p className="absolute bottom-7 left-7 right-7 font-['Cormorant_Garamond'] text-3xl leading-none text-white">A small welcome,<br/>made for living.</p></div>
            <div className="relative p-6 md:p-8"><Dialog.Close aria-label="Close welcome offer" className="absolute right-4 top-4 grid size-10 place-items-center rounded-full border border-black/8"><X size={17}/></Dialog.Close><span className="grid size-12 place-items-center rounded-full bg-[#e4eadc] text-[#214d2b]"><Gift size={21}/></span><Dialog.Title className="mt-5 font-['Cormorant_Garamond'] text-[42px] leading-[.9]">Welcome to .CO</Dialog.Title><Dialog.Description className="mt-4 text-sm leading-6 text-[#655a52]">Claim an exclusive <strong className="text-[#214d2b]">50% off your first purchase</strong>. Quietly useful, never noisy.</Dialog.Description>
              {claimed ? <div className="mt-7 rounded-[22px] bg-[#e5ecdf] p-5 text-[#214d2b]" role="status"><Check size={20}/><p className="mt-3 font-semibold">Your welcome is saved.</p><p className="mt-1 text-xs">Taking you to create your .CO account…</p></div> : <form className="mt-7 space-y-3" onSubmit={(event) => { event.preventDefault(); if (!event.currentTarget.reportValidity()) return; window.localStorage.setItem(welcomeKey, "claimed"); document.cookie = "co_welcome_claimed=1; Max-Age=31536000; Path=/; SameSite=Lax; Secure"; setClaimed(true); window.setTimeout(() => router.push("/register?source=welcome"), 700); }}>
