...
export function ContactQuickForm() {
  ...
  return <form id="contact-form" className="scroll-mt-28 rounded-[28px] border border-black/6 bg-[#f8f4ec] p-5 md:p-7" data-analytics-form="contact" onSubmit={(event)=>{event.preventDefault();if(!event.currentTarget.reportValidity())return;const data=new FormData(event.currentTarget);const subject=encodeURIComponent(`.CO enquiry - ${String(data.get("topic"))}`);const body=encodeURIComponent(`Name: ${String(data.get("name"))}\nEmail: ${String(data.get("email"))}\n\n${String(data.get("message"))}`);setStatus("Your email app is opening with the message ready for your review. This website has not stored the enquiry.");window.location.href=`mailto:hello@cothecoconutcompany.com?subject=${subject}&body=${body}`;}}>
