    ...
    <section className="relative isolate overflow-hidden border-b border-[var(--co-border)] bg-[var(--co-cream)]">
      <div className="co-container relative z-10 grid min-h-[calc(100dvh-88px)] items-center gap-8 py-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-10 lg:py-16">
        <div className="relative max-w-2xl">
          <motion.p initial={{ opacity: shouldReduce ? 1 : 0, y: shouldReduce ? 0 : 18 }} animate={{ opacity: 1, y: 0 }} transition={transition} className="mb-5 text-xs font-bold uppercase tracking-[0.14em] text-[var(--co-palm)] md:text-sm">
          ...
          >
            {["Cold", "Coconut", "Water."].map((line) => (
              <motion.span key={line} variants={{ hidden: { opacity: shouldReduce ? 1 : 0, y: shouldReduce ? 0 : 52 }, show: { opacity: 1, y: 0 } }} transition={transition} className="block">
                {line}
              </motion.span>
            ))}
          </motion.h1>
          ...
          >
            ...
            <CTAButton href="/about" variant="outline">Explore Story</CTAButton>
          </motion.div>
          ...
          >
            {[<TrustBadge key="leaf" icon="leaf" title="100% Natural" body="Real coconut taste." />, <TrustBadge key="drop" icon="drop" title="Nothing Added" body="Clean and simple." />, <TrustBadge key="cold" icon="cold" title="Fridge Shelf Ready" body="Best served cold." />].map((badge, index) => (
              <motion.div key={index} variants={{ hidden: { opacity: shouldReduce ? 1 : 0, y: shouldReduce ? 0 : 16 }, show: { opacity: 1, y: 0 } }} transition={transition}>
                {badge}
              </motion.div>
            ))}
          </motion.div>
        ...
        <motion.div
          style={{ y: mediaY }}
          initial={{ opacity: shouldReduce ? 1 : 0, clipPath: shouldReduce ? "inset(0% 0% 0% 0% round 40px)" : "inset(0% 0% 18% 0% round 40px)", scale: shouldReduce ? 1 : 0.985 }}
