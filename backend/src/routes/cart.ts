...
export const cartRoutes = new Hono<AppBindings>()
  ...
  .post("/wishlist/items", requireAuth, async (c) => { const body = productIdParamSchema.parse(await c.req.json()); const current = await getWishlist(c.get("user")!.userId); return c.json({ data: await saveWishlist(c.get("user")!.userId, Array.from(new Set([...current.productIds, body.productId]))), meta: {}, requestId: c.get("requestId") }, 202); })
  .delete("/wishlist/items/:productId", requireAuth, async (c) => { const body = productIdParamSchema.parse(c.req.param()); const current = await getWishlist(c.get("user")!.userId); return c.json({ data: await saveWishlist(c.get("user")!.userId, current.productIds.filter((id) => id !== body.productId)), meta: {}, requestId: c.get("requestId") }); });
