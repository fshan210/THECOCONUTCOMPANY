...
import { ConfirmForgotPasswordCommand, ConfirmSignUpCommand, ForgotPasswordCommand, InitiateAuthCommand, ResendConfirmationCodeCommand, SignUpCommand, CognitoIdentityProviderClient } from "@aws-sdk/client-cognito-identity-provider";
import { z } from "zod";
...
const input = z.object({ action: z.enum(["login", "signup", "confirm", "resend", "forgot", "reset", "logout"]), email: z.string().email().optional(), password: z.string().min(8).optional(), name: z.string().min(2).max(80).optional(), code: z.string().min(4).max(12).optional(), clientId: z.string().optional(), refreshToken: z.string().optional() });
...
export async function POST(request: Request) {
  ...
  try {
    ...
    store.set(cookieName, sealAwsSession({ accessToken: auth.AccessToken, idToken: auth.IdToken, refreshToken: auth.RefreshToken, expiresAt: Math.floor(Date.now() / 1000) + (auth.ExpiresIn || 900) }), { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: auth.ExpiresIn || 900 });
    return NextResponse.json({ ok: true });
  } catch (error) {
